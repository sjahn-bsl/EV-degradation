...
def calculate_SOH_OCV(file_path):
    try:
        ...
        if abs(delta_SOC_OCV) < 0.4:
            return None  # SOC 변화량이 너무 적으면 제외

        # 주어진 Trip동안 사용된 전류 데이터를 trapz로 적분하여 Q_current 계산
        # extended 파일의 경우, Trip 파일의 마지막 행이 추가된 Rest 행이므로, 마지막 행 값은 제외하기 위해 df['time'].iloc[:-1], df['cell_current'].iloc[:-1]
        df['time_seconds'] = (df['time'] - df['time'].iloc[0]).dt.total_seconds()
        Q_current = np.trapz(df['cell_current'], df['time_seconds']) / (delta_SOC_OCV) / 3600  # Ah 변환, 위와 같은 이유
