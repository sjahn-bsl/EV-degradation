...
def calculate_loss(file_path):
    ...
    cyc_low_T_losses, cyc_low_T_high_SOC_losses = [], []
    total_losses = []

    chr_cap, cap = 0.0, 0.0
    prev_time = 0.0
    prev_chr_cap, prev_cap = 0.0, 0.0
    time_list = []
    ...
    for i in tqdm(range(len(df)), desc=f"Processing {os.path.basename(file_path)}"):
        ...
        calendar_loss, _ = quad(
            lambda t: integrate_k_cal(t, T_row, SOC), prev_time, time_hr
        )
        cyc_high_T_loss, _ = quad(
            lambda phi_tot: integrate_k_cycHighT(phi_tot, T_row), prev_cap, cap
        )
        if current > 0:  # 충전일 때만
            cyc_low_T_loss, _ = quad(
                lambda phi_ch: integrate_k_cycLowT(phi_ch, T_row, current),
                prev_chr_cap,
                chr_cap,
            )
            if SOC > 80:
                cyc_low_T_high_SOC_loss, _ = quad(
                    lambda phi_ch: integrate_k_cycLowTHighSOC(T_row, current, SOC),
                    prev_chr_cap,
                    chr_cap,
                )
            else:
        ...
        total_loss = (
            calendar_loss + cyc_high_T_loss + cyc_low_T_loss + cyc_low_T_high_SOC_loss
        )
...
plt.plot(
    result_NONDFC["time"], result_NONDFC["total"] * 100, label="NON DFC", color="red"
)
plt.xlabel("Time [h]")
...
DFC_vals = [
    result_DFC["cal"][-1],
    result_DFC["cyc_high"][-1],
    result_DFC["cyc_low"][-1],
    result_DFC["cyc_highSOC"][-1],
]
NONDFC_vals = [
    result_NONDFC["cal"][-1],
    result_NONDFC["cyc_high"][-1],
    result_NONDFC["cyc_low"][-1],
    result_NONDFC["cyc_highSOC"][-1],
]
...
for label, result in [("DFC", result_DFC), ("NON DFC", result_NONDFC)]:
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, f"Loss_Components_vs_Time_{label}.png"), dpi=300)
    plt.close()
