...
def load_merged_data(merged_dir):
    """
    Merged 폴더의 모든 CSV 데이터를 한 번만 로드하여 캐싱.
    이후 get_future_data() 및 get_closest_future_data()는 이 데이터를 활용.
    """
    merged_data = {}  # 장치별 데이터 저장

    file_list = [f for f in os.listdir(merged_dir) if f.endswith(".csv")]

    for file in file_list:
        file_path = os.path.join(merged_dir, file)
        name_parts = file.split('_')  # 파일명을 '_' 기준으로 분할

        # altitude 데이터가 포함된 경우 처리
        if "altitude" in file:
            device_no = name_parts[2]  # altitude가 포함된 경우 device_no 위치가 다름
        else:
            device_no = name_parts[1]  # 일반적인 경우 device_no 위치

        df = pd.read_csv(file_path, low_memory=False, usecols=["time", "pack_volt"],
                         dtype={"time": str, "pack_volt": "float32"})
        df['time'] = pd.to_datetime(df['time'])

        if device_no not in merged_data:
            merged_data[device_no] = df
        else:
            merged_data[device_no] = pd.concat([merged_data[device_no], df], ignore_index=True)

    return merged_data

def process_trip_by_trip_soc_2hr_extended(start_path, save_path):
    """
    ...
    with tqdm(total=total_files, desc="Processing files", unit="file") as pbar: #진행률바를 출력하는 라이브러리
        ...
        with ProcessPoolExecutor(max_workers=8) as executor: # 병렬 처리(Python 멀티프로세싱) 지원
            for file_path in csv_files: # 각 csv파일을 병렬로 처리
                future = executor.submit(process_wrapper_2hr_extended, file_path, save_path) # 각 csv 파일을 process_wrapper() 함수에 전달하여 비동기 실행, 비동기 실행=여러 파일을 동시에 처리
...
def process_wrapper_2hr_extended(file_path, save_path):
    """
    단일 파일을 처리하는 래퍼 함수 (SOC 추정용 2시간 확장 조건 적용)
    """
    try:
        process_single_file_2hr_extended(file_path, save_path) # process_single_file 호출
    except Exception as e: # 실행 도중 오류가 발생할 경우 예외 처리
        print(f"Error processing file {file_path}: {e}")
        raise # raise를 사용하여 예외를 다시 발생시킴


def get_future_data(merged_data, device_no, future_time):
    """
    미리 로드한 merged_data에서 2시간 후 데이터를 검색하여 반환.
    """
    if device_no in merged_data:
        df = merged_data[device_no]  # 미리 메모리에 로드된 데이터 사용
        future_data = df[df['time'] == future_time]  # 메모리에서 바로 검색
        if not future_data.empty:
            return future_data.iloc[0]
    return None  # 데이터가 없으면 None 반환


def get_closest_future_data(merged_dir, device_no, last_time, min_seconds=7200):
    """
    Merged 폴더에서 last_time 이후 min_seconds(7200초) 이상인 데이터 중 가장 가까운 데이터를 반환
    """
    closest_future_data = None
    closest_time_diff = float('inf')

    for root, _, files in os.walk(merged_dir):
        for file in files:
            if file.endswith(".csv") and device_no in file:
                file_path = os.path.join(root, file)
                df = pd.read_csv(file_path)
                df['time'] = pd.to_datetime(df['time'])

                # 최소 2시간 이후 데이터 선택
                future_candidates = df[df['time'] >= last_time + pd.Timedelta(seconds=min_seconds)]

                if not future_candidates.empty:
                    closest_candidate = future_candidates.iloc[0]  # 가장 가까운 이후 데이터 선택
                    time_diff = (closest_candidate['time'] - last_time).total_seconds()

                    if time_diff < closest_time_diff:
                        closest_time_diff = time_diff
                        closest_future_data = closest_candidate.copy()

    return closest_future_data  # 가장 가까운 이후 데이터 반환 (없으면 None)


def process_single_file_2hr_extended(file_path, save_path):
    """
    ...
    try: #예외 발생 시 프로그램이 중단되지 않도록 처리
        ...
        for idx in range(len(cut) - 1): # 분할된 Trip 개수만큼 반복
            ...
            # (5-1) Trip 기본 유효성 체크
            if not check_trip_base_conditions_2hr_extended(trip):
                continue # 유효하지 않은 Trip이면 처리하지 않고 넘어감

            # (5-2) Trip 확장 조건 + expand(앞뒤 30행)
            expanded_trip = check_time_gap_conditions_2hr_extended(data, start_idx, end_idx)
            if expanded_trip is None:
                continue # 확장 불가능한 Trip은 저장하지 않음

            ###################################################################
            # SettingWithCopyWarning 방지 코드
            ###################################################################
            expanded_trip = expanded_trip.copy()  # DataFrame의 복사본을 생성하여 안전하게 수정
            expanded_trip['time'] = pd.to_datetime(expanded_trip['time'])  # 시간 형식을 변환
            ...
            # 2시간 후 데이터 추가 기능 (이 부분 추가)
            ###################################################################
            expanded_trip['time'] = pd.to_datetime(expanded_trip['time'])
            last_time = expanded_trip['time'].iloc[-1]
            future_time = last_time + pd.Timedelta(seconds=7200)  # 2시간 후 시간 계산

            # Merged 폴더에서 2시간 후 데이터 찾기 (가장 가까운 데이터 포함)
            merged_path = os.path.normpath(r'D:\SamsungSTF\Processed_Data\Merged')
            future_data = get_future_data(merged_path, device_no, future_time)

            # 정확히 2시간 후 데이터가 없으면, seconds >= 7200 중 가장 가까운 데이터 선택
            if future_data is None:
                future_data = get_closest_future_data(merged_path, device_no, last_time, min_seconds=7200)

            if future_data is not None:
                new_row = future_data.copy()  # 가장 가까운 이후 데이터를 추가
            else:
                new_row = expanded_trip.iloc[-1].copy()
                new_row['time'] = future_time  # 시간만 변경하여 추가

            # 새로운 행 추가
            expanded_trip = pd.concat([expanded_trip, pd.DataFrame([new_row])], ignore_index=True)
            ...
            # (5-4) 최종 Trip 저장
            filename = f"{file_prefix}{trip_counter}_extended.csv"
            expanded_trip.to_csv(os.path.join(vehicle_save_path, filename), index=False, encoding='utf-8-sig')
...
def check_trip_base_conditions_2hr_extended(trip):
    """
...
def check_time_gap_conditions_2hr_extended(data, start_idx, end_idx):
    """
