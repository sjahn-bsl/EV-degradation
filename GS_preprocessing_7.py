...
def process_trip_by_trip_soc_2hr_with_charging_rest(start_path, save_path, save_path2, save_path3):
    """
    ...
    with tqdm(total=total_files, desc="Processing files", unit="file") as pbar: #진행률바를 출력하는 라이브러리
        ...
        with ProcessPoolExecutor() as executor: # 병렬 처리(Python 멀티프로세싱) 지원
            for file_path in csv_files: # 각 csv파일을 병렬로 처리
                future = executor.submit(process_wrapper_2hr_with_charging_rest, file_path, save_path, save_path2, save_path3) # 각 csv 파일을 process_wrapper() 함수에 전달하여 비동기 실행, 비동기 실행=여러 파일을 동시에 처리
...
def process_wrapper_2hr_with_charging_rest(file_path, save_path, save_path2, save_path3):
    """
    단일 파일을 처리하는 래퍼 함수 (SOC 추정용 2시간 확장 조건 적용)
    """
    try:
        process_single_file_2hr_with_charging_rest(file_path, save_path, save_path2, save_path3) # process_single_file 호출
...
def process_single_file_2hr_with_charging_rest(file_path, save_path, save_path2, save_path3):
    """
    ...
    try: #예외 발생 시 프로그램이 중단되지 않도록 처리
        ...
        try:
            ...
            # 더 정확한 정규표현식으로 device_id 추출
            match = re.search(r'bms(?:_altitude)?_(\d+)_\d{4}-\d{2}', file_name)
            if match:
                ...
                if vehicle_model is not None:
                    ...
                    if parallel_count is not None:
                        data['cell_current'] = data['pack_current'] / parallel_count
                    else:
                        print(f"[Warning] {file_name}: 병렬수(parallel count) 정보를 찾을 수 없습니다.")
                        data['cell_current'] = np.nan
                else:
                    print(f"[Warning] {file_name}: device_id {device_id}가 vehicle_dict에서 찾을 수 없습니다.")
                    data['cell_current'] = np.nan
            else:
                print(f"[Warning] {file_name}: device_id를 추출할 수 없습니다.")
                data['cell_current'] = np.nan
        except Exception as e:
            print(f"[ERROR] cell_current 계산 실패: {e}")
            data['cell_current'] = np.nan
        ...
        rest_save_path = os.path.join(save_path3, vehicle_type)
        os.makedirs(vehicle_save_path, exist_ok=True)
        os.makedirs(charging_save_path, exist_ok=True)
        os.makedirs(rest_save_path, exist_ok=True)
        ...
        rest_trip_counter = 1 # Rest Trip 카운터 추가
        ...
        for idx in range(len(cut) - 1): # 분할된 Trip 개수만큼 반복
            ...
            if data.loc[start_idx, 'chrg_cable_conn'] != 0:
                if skip_chrg:
                    continue
                # 충전 조건 검사
                if check_charge_conditions(trip):
                ...
                else: # 충전 조건 불충족 시 Rest check로 이동
                    if check_rest_conditions(trip):
                        rest_filename = f"{file_prefix}rest-{rest_trip_counter}.csv"
                        trip.to_csv(os.path.join(rest_save_path, rest_filename), index=False)
                        print(f"Rest Trip saved: {os.path.join(vehicle_type, rest_filename)}")
                        rest_trip_counter += 1
                    continue  # 충전 Trip 판단 끝난 경우 다음 루프 진행

            if skip_drive:
                continue

            # (5-1) Trip 기본 유효성 체크
            if check_trip_base_conditions_2hr_with_charging_rest(trip):

                # (5-2) Trip 확장 조건 + expand(앞뒤 30행)
                expanded_trip = check_time_gap_conditions_2hr_with_charging_rest(data, start_idx, end_idx)

                # (5-3) "확장된 Trip" 평균 모듈 온도도 20~28℃ 범위인지 재확인
                if expanded_trip is not None:
                    expanded_temp_mean = expanded_trip['mod_temp_avg'].mean()
                    if 20 <= expanded_temp_mean <= 28:
            ...
            else:  # 기본 주행 조건 불충족 시 Rest 체크
                if check_rest_conditions(trip):
                    rest_filename = f"{file_prefix}rest-{rest_trip_counter}.csv"
                    trip.to_csv(os.path.join(rest_save_path, rest_filename), index=False)
                    print(f"Rest Trip saved: {os.path.join(vehicle_type, rest_filename)}")
                    rest_trip_counter += 1 # 유효하지 않은 Trip이면 처리하지 않고 넘어감
...
def check_trip_base_conditions_2hr_with_charging_rest(trip):
    """
...
def check_time_gap_conditions_2hr_with_charging_rest(data, start_idx, end_idx):
    """
...
def check_rest_conditions(trip):
    try:
        threshold = 0.05 * 56.168  # 2.8084A
        trip = trip.copy().reset_index(drop=True)

        if not pd.api.types.is_datetime64_any_dtype(trip['time']):
            trip['time'] = pd.to_datetime(trip['time'], errors='coerce')

        # Step 1. Rest 조건 (cell_current < 2.8084A) 검사
        trip['resting'] = (trip['cell_current'].abs() < threshold).astype(int)

        # 5분(300초) 이상 resting 유지 구간 검사
        trip['resting_shift'] = trip['resting'].shift().fillna(0)
        trip['change'] = trip['resting'] != trip['resting_shift']
        change_indices = trip.index[trip['change']].tolist() + [trip.index[-1]]
        ...
        if not rest_ok:
            return False  # 기본 Rest 조건도 불통과

        # Step 2. unrest 검사: 전체 trip 동안 2.8084A 이상 샘플 개수 세기
        unrest_condition = (trip['cell_current'].abs() >= threshold)
        unrest_count = unrest_condition.sum()

        # 5분 동안 2초 간격 → 150개 허용
        unrest_limit = 150

        if unrest_count >= unrest_limit:
            return False  # 2.8084A 이상 포인트가 너무 많으면 Trip 삭제

        return True

    except Exception as e:
        print("[ERROR] check_rest_conditions() 실패:", e)
        return False
