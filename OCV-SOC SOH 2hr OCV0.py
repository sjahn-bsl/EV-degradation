...
trip_data_folder = r"E:\SamsungSTF\Processed_Data\TripByTrip_soc_2hr"
trip_files = []
...
def calculate_SOH_OCV(file_path, threshold):
    try:
        ...
        trip_start_time = df['time'].iloc[30]  # 31번째 time 값
        soh_initial = df['soh'].iloc[30] if 'soh' in df.columns else None # 31번째 SOH 값
        odometer_initial = df['odometer'].iloc[30] if 'odometer' in df.columns else None  # 31번째 odometer 값

        # Trip 구간만 추출
        df_trip = df.iloc[30:-30].copy()

        # pack_volt0: Trip 시작 시점
        pack_volt0 = df['pack_volt'].iloc[30]

        # pack_volt1: Trip 종료 시점 (단, 전류가 충분히 작을 때만)
        current_threshold = 0.1 * 56.168
        pack_volt1 = df['pack_volt'].iloc[-1]  # 기본값

        for i in range(1, 31):
            if abs(df['cell_current'].iloc[-i]) < current_threshold:
                pack_volt1 = df['pack_volt'].iloc[-i]
                break
        ...
        if soc_initial == 0:
            ...
            if not non_zero_soc.empty:
                soc_initial = non_zero_soc.iloc[30]
            else:
                return {"file": file_name, "error": "SOC values are all zero"}
        # SOC_BMS 최종값 설정
        soc_end = df['soc'].iloc[-31]
        if soc_end == 0:
            non_zero_soc = df['soc'][df['soc'] > 0]
            if not non_zero_soc.empty:
        ...
        Q_current = np.trapz(df_trip['cell_current'], df_trip['time_seconds']) / -(delta_SOC_OCV) / 3600  # Ah 변환
        ...
        return {
            ...
            "pack_volt0": pack_volt0, "pack_volt1": pack_volt1, "OCV0": OCV0, "OCV1": OCV1,
            "SOC0": SOC0, "SOC1": SOC1, "BMS_SOC0": soc_initial, "BMS_SOC1": soc_end,
            "delta_SOC": delta_SOC_OCV, "delta_BMS_SOC": delta_BMS_SOC, "diff delta_SOC (%)": 100*(delta_SOC_OCV - delta_BMS_SOC),
...
if __name__ == "__main__":
    ...
    for threshold, results in results_by_threshold.items():
        ...
        output_csv_path = fr"C:\Users\BSL\Desktop\EV 열화 인자 분석\250306\OCV-SOC SOH results 2hr OCV0 {int(threshold * 100)}%.csv"
