...
# 그래프를 저장할 새로운 폴더 설정
output_folder = os.path.join(base_folder, "SOH_BMS vs Odometer")
...
for delta in SOC_deltas:
    ...
    output_file = os.path.join(output_folder, f"SOH_BMS vs Odometer {delta}.png")
    ...
    for device_id in unique_devices:
        ...
        plt.plot(subset["odometer (km)"], subset["SOH_BMS (%)"], marker='o', linestyle='-', label=f"Device {device_id}")

    # 그래프 설정
    plt.xlabel("Odometer (km)")
    plt.ylabel("SOH_BMS (%)")
    plt.title(f"SOH_BMS vs Odometer per Device (Total Devices: {total_devices}, ΔSOC={delta})")  # ΔSOC 정보 추가
